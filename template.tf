...
resource "proxmox_virtual_environment_vm" "netbox" { #описание ресурса в виде ресурс "вид ресурса" "название"
  count       = 1             # Счетчик цикла. Цикл используется для создания сразу нескольких ВМ
  name        = var.vm_hostname   # Имя виртуальной машины
  description = "Managed by Terraform" # Описание ВМ
  ...
  cpu {                       # Секция для настроек ЦПУ
    cores      = 4            # Количество виртуальных ядер (vCPU)
  ...
  memory {                    # Секция для настроек оперативной памяти
    dedicated = 8192          # Количество выделяемой памяти в Mb
  }
  ...
  startup {                   # Секция для настроек запуска ВМ
    order      = "3"          # Порядок запуска. Машина с наименьшим числом запускается первой
    up_delay   = "60"         # Задержка перед запуском
    down_delay = "60"         # Задержка перед выключением
  }
  efi_disk {
  ...
  disk {                      # Секция для настройки жесткого диска ВМ. Для добавления второго диска добавить еще секцию disk и увеличить счетчик интерфеса
    datastore_id = var.storage_pool # Имя хранилища. Хранилище должно быть активно на ноде, на которой создаем ВМ
    file_id      = "local:iso/jammy-server-cloudimg-amd64.img" # ИСО файл для установки ОС (облачной конфигурации). Должен лежать на локальном хранилище.
